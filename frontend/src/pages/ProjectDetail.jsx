...
function SessionReportModal({ sessionId, onClose }) {
  ...
  const handlePrint = () => {
    ...
    w.document.write(`<html><head><title>Session Report — ${sessionId.slice(0,8)}</title>
  ...
  if (loading) return (
    <div style={s.overlay}>
      <div style={{ background: 'white', borderRadius: 16, padding: 60, textAlign: 'center', fontFamily: FONT, color: '#64748b' }}>
  ...
  if (!detail) return (
    <div style={s.overlay}>
      <div style={{ background: 'white', borderRadius: 16, padding: 32, maxWidth: 440, width: '100%', textAlign: 'center' }}>
        <p style={{ fontFamily: FONT, color: '#64748b' }}>Session details not available yet.</p>
        <button style={s.cancelBtnFull} onClick={onClose}>Close</button>
      </div>
    </div>
  );
  ...
  return (
    <div style={{ ...s.overlay, alignItems: 'flex-start', paddingTop: 20 }}>
      <div style={{ background: 'white', borderRadius: 16, width: '100%', maxWidth: 1020, maxHeight: '94vh', display: 'flex', flexDirection: 'column', boxShadow: '0 25px 50px rgba(0,0,0,0.3)' }}>
        ...
        <div style={{ padding: '20px 24px', borderBottom: '1px solid #f1f5f9', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 4 }}>
              ...
              <span style={{ fontFamily: 'monospace', fontSize: '0.82rem', color: '#64748b', background: '#f1f5f9', padding: '2px 8px', borderRadius: 4 }}>{session.id.slice(0,8)}</span>
              {session.outcome && (
                <span style={{ background: oc.bg, color: oc.text, borderRadius: 20, padding: '3px 10px', fontSize: '0.75rem', fontWeight: 600, fontFamily: FONT }}>{formatLabel(session.outcome)}</span>
              )}
            </div>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button onClick={handlePrint} style={{ display: 'flex', alignItems: 'center', gap: 6, background: '#f0f7ff', border: '1.5px solid #dbeafe', borderRadius: 8, padding: '7px 14px', fontSize: '0.82rem', fontWeight: 600, cursor: 'pointer', color: '#1e3a5f', fontFamily: FONT }}>
        ...
        <div style={{ flex: 1, overflow: 'hidden', display: 'flex' }}>
          ...
          {/* Left panel */}
          <div style={{ width: 210, borderRight: '1px solid #f1f5f9', overflowY: 'auto', flexShrink: 0 }}>
            ...
            {pageEvents.length === 0
              ? <div style={{ padding: '12px 14px', fontSize: '0.82rem', color: '#94a3b8', fontFamily: FONT }}>No pages recorded yet.</div>
              : pageEvents.map((ev, i) => (
                <button key={i} onClick={() => setActivePageIdx(i)} style={{ width: '100%', textAlign: 'left', padding: '10px 14px', border: 'none', background: activePageIdx === i ? '#f0f7ff' : 'transparent', borderLeft: activePageIdx === i ? '3px solid #2563eb' : '3px solid transparent', cursor: 'pointer', display: 'flex', flexDirection: 'column', gap: 3 }}>
                  <div style={{ fontSize: '0.82rem', fontWeight: 600, color: activePageIdx === i ? '#1e3a5f' : '#1e293b', fontFamily: FONT }}>Page {i + 1}</div>
                  <div style={{ fontSize: '0.72rem', color: '#94a3b8', fontFamily: FONT }}>{fmtDuration(ev.payload?.timeTaken)}</div>
                  {ev.payload?.screenshot && (
                    <div style={{ width: '100%', height: 54, background: '#f8fafc', borderRadius: 4, overflow: 'hidden', marginTop: 4, border: '1px solid #e2e8f0' }}>
                      <img
            ...
            {session.outcome && (
              <div style={{ margin: '8px 14px', background: oc.bg, borderRadius: 6, padding: '8px 10px' }}>
                <div style={{ fontSize: '0.75rem', fontWeight: 700, color: oc.text, fontFamily: FONT }}>
                  {session.outcome === 'completed' ? '✅ Complete' : session.outcome === 'terminated' ? '⛔ Terminate' : session.outcome === 'over_quota' ? '📊 Over Quota' : '❌ Error'}
                </div>
              </div>
            )}
          </div>
          ...
          {/* Right panel */}
          <div style={{ flex: 1, overflowY: 'auto', padding: 24 }} ref={printRef}>

            {session?.outcome === 'error' && errorText && (
              <div style={{ marginBottom: 16, background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 10, padding: '10px 12px' }}>
                <div style={{ fontFamily: FONT, fontSize: '0.78rem', fontWeight: 700, color: '#991b1b', marginBottom: 4 }}>Error details</div>
                <div style={{ fontFamily: 'monospace', fontSize: '0.75rem', color: '#7f1d1d', whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>{String(errorText).slice(0, 2000)}</div>
              </div>
            )}

            {pageEvents.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '60px 0', color: '#94a3b8', fontFamily: FONT }}>
                <Camera size={48} color="#e2e8f0" style={{ marginBottom: 12 }} />
                <div style={{ fontWeight: 600, marginBottom: 6 }}>Session data not yet available</div>
              ...
              <div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
                  <h3 style={{ fontFamily: FONT, fontSize: '1rem', fontWeight: 700, color: '#1e293b', margin: 0 }}>
                    Page {activePageIdx + 1} — {activePage.payload?.title || 'Survey Page'}
                  </h3>
                  <div style={{ display: 'flex', gap: 8 }}>
                    <span style={{ fontSize: '0.75rem', color: '#64748b', fontFamily: FONT, background: '#f1f5f9', padding: '3px 8px', borderRadius: 6 }}>⏱ {fmtDuration(activePage.payload?.timeTaken)}</span>
                    <span style={{ fontSize: '0.72rem', color: '#94a3b8', fontFamily: FONT, background: '#f1f5f9', padding: '3px 8px', borderRadius: 6 }}>{fmtTimeShort(activePage.created_at)}</span>
                  </div>
                </div>
                ...
                {/* Screenshot */}
                <div style={{ marginBottom: 20, border: '1.5px solid #e2e8f0', borderRadius: 10, overflow: 'hidden', background: '#f8fafc' }}>
                  <div style={{ padding: '8px 12px', borderBottom: '1px solid #e2e8f0', fontSize: '0.75rem', fontWeight: 700, color: '#94a3b8', fontFamily: FONT, textTransform: 'uppercase', letterSpacing: 0.5, display: 'flex', alignItems: 'center', gap: 6 }}>
                  ...
                  {!imgError[activePageIdx] ? (
                    <img
                      ...
                      onError={() => setImgError(p => ({ ...p, [activePageIdx]: true }))}
                      alt={`Screenshot of page ${activePageIdx + 1}`}
                    />
                  ) : (
                    <div style={{ padding: 32, textAlign: 'center', color: '#94a3b8', fontFamily: FONT, fontSize: '0.85rem' }}>
                      <Camera size={32} color="#e2e8f0" style={{ marginBottom: 8 }} /><br />Screenshot not available
                    </div>
                  )}
                </div>
                ...
                {activePage.payload?.questions?.length > 0 && (
                  <div style={{ marginBottom: 16 }}>
                    <div style={{ fontSize: '0.75rem', fontWeight: 700, color: '#94a3b8', fontFamily: FONT, textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 8 }}>Questions Detected</div>
                    {activePage.payload.questions.map((q, qi) => (
                      <div key={qi} style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 8, padding: '10px 12px', marginBottom: 6, fontSize: '0.85rem', color: '#1e293b', fontFamily: FONT, lineHeight: 1.5 }}>
                ...
                {activePage.payload?.options?.length > 0 && (
                  <div style={{ marginBottom: 16 }}>
                    <div style={{ fontSize: '0.75rem', fontWeight: 700, color: '#94a3b8', fontFamily: FONT, textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 8 }}>All Options & Selection</div>
                    {activePage.payload.options.map((optGroup, gi) => (
                      <div key={gi} style={{ background: 'white', border: '1.5px solid #e2e8f0', borderRadius: 10, padding: 14, marginBottom: 12 }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                          <span style={{ fontSize: '0.7rem', fontWeight: 700, background: '#f0f7ff', color: '#2563eb', borderRadius: 6, padding: '2px 8px', fontFamily: FONT, textTransform: 'uppercase', letterSpacing: 0.4 }}>
                            {optGroup.type}
                          </span>
                          {optGroup.type === 'radio' && optGroup.selected && (
                            <span style={{ fontSize: '0.75rem', color: '#166534', fontFamily: FONT }}>
                              ✓ Selected: <strong>{optGroup.selected}</strong>
                            </span>
                          )}
                          {optGroup.type === 'checkbox' && optGroup.selected?.length > 0 && (
                            <span style={{ fontSize: '0.75rem', color: '#166534', fontFamily: FONT }}>
                              ✓ {optGroup.selected.length} of {optGroup.options?.length} selected
                            </span>
                          )}
                        ...
                        {optGroup.options?.length > 0 && (
                          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                            {optGroup.options.map((opt, oi) => {
                              const isSelected = optGroup.type === 'checkbox'
                              ...
                              return (
                                <div key={oi} style={{
                                  ...
                                  <div style={{
                                    width: 16, height: 16, borderRadius: optGroup.type === 'checkbox' ? 4 : '50%',
                                  ...
                                  <span style={{ fontSize: '0.83rem', color: isSelected ? '#166534' : '#475569', fontFamily: FONT, fontWeight: isSelected ? 600 : 400, flex: 1 }}>
                                    {opt}
                                  </span>
                                  {isSelected && (
                                    <span style={{ fontSize: '0.7rem', background: '#059669', color: 'white', borderRadius: 4, padding: '1px 6px', fontFamily: FONT, fontWeight: 700 }}>
                                      SELECTED
                                    </span>
                                  )}
                                  <span style={{ fontSize: '0.7rem', color: '#94a3b8', fontFamily: FONT }}>
                        ...
                        {(optGroup.type === 'open-end' || optGroup.type === 'numeric') && optGroup.selected && (
                          <div style={{ background: '#f0fdf4', border: '1px solid #86efac', borderRadius: 6, padding: '8px 12px', fontSize: '0.85rem', color: '#166534', fontFamily: FONT }}>
                            {optGroup.selected}
                          </div>
                        )}
                      </div>
                ...
                {activePage.payload?.answers?.some(a => a?.type === 'open-end') && (
                  <div style={{ marginBottom: 16 }}>
                    <div style={{ fontSize: '0.75rem', fontWeight: 700, color: '#94a3b8', fontFamily: FONT, textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 8 }}>Open-End Response</div>
                    {activePage.payload.answers.filter(a => a?.type === 'open-end').map((a, ai) => (
                      <div key={ai} style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: 8, padding: '10px 12px', marginBottom: 6, fontSize: '0.85rem', color: '#166534', fontFamily: FONT }}>
                        {a.text}
                      </div>
                    ))}
                  </div>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 20, paddingTop: 16, borderTop: '1px solid #f1f5f9' }}>
                  <button onClick={() => setActivePageIdx(i => Math.max(0, i - 1))} disabled={activePageIdx === 0}
                    style={{ background: 'none', border: '1.5px solid #e2e8f0', borderRadius: 8, padding: '7px 14px', fontSize: '0.85rem', cursor: activePageIdx === 0 ? 'not-allowed' : 'pointer', color: '#64748b', fontFamily: FONT, opacity: activePageIdx === 0 ? 0.4 : 1 }}>
                    ← Previous Page
                  </button>
                  <span style={{ fontSize: '0.8rem', color: '#94a3b8', fontFamily: FONT, alignSelf: 'center' }}>
                    {activePageIdx + 1} / {pageEvents.length}
                  </span>
                  <button onClick={() => setActivePageIdx(i => Math.min(pageEvents.length - 1, i + 1))} disabled={activePageIdx === pageEvents.length - 1}
                    style={{ background: '#1e3a5f', border: 'none', borderRadius: 8, padding: '7px 14px', fontSize: '0.85rem', cursor: activePageIdx === pageEvents.length - 1 ? 'not-allowed' : 'pointer', color: 'white', fontFamily: FONT, opacity: activePageIdx === pageEvents.length - 1 ? 0.4 : 1 }}>
                    Next Page →
                  </button>
                </div>
              </div>
            ) : (
              <div style={{ textAlign: 'center', padding: 40, color: '#94a3b8', fontFamily: FONT }}>Select a page from the left panel</div>
            )}
          </div>
        </div>
