...
const {
  getProjectScenarios, getScenarioById, createScenario,
  updateScenario, deleteScenario, duplicateScenario,
...
router.post('/from-session/:sessionId', requireRole('admin', 'project_manager'), async (req, res) => {
  try {
    const { name, description, expectedOutcome, projectId, selectedPageIndices } = req.body;
    ...
    pageEvents.forEach((ev, i) => {
      ...
      options.forEach(optGroup => {
        if (!optGroup.selected || (Array.isArray(optGroup.selected) && optGroup.selected.length === 0)) return;
        ...
        if (optGroup.type === 'radio' && optGroup.selected) {
          ...
          action = 'select_exact';
        } else if (optGroup.type === 'checkbox' && optGroup.selected?.length > 0) {
        ...
        if (action === 'open_end') {
          ...
          steps.push({
            when_type:     questionText ? 'question_contains' : 'page_number',
            when_value:    questionText ? questionText.slice(0, 120) : String(i + 1),
            conditions:    [],
            action,
    ...
    const scenario = await createScenario({
      ...
      workspaceId:      req.user.workspace_id,
      name,
      description:      description || `Created from session ${req.params.sessionId.slice(0, 8)}`,
...
router.patch('/:id', requireRole('admin', 'project_manager'), async (req, res) => {
  try {
    ...
    const scenario = await updateScenario(req.params.id, { name, description, expectedOutcome, isActive, steps });
    res.json({ scenario });
  } catch (err) {
...
router.post('/:id/duplicate', requireRole('admin', 'project_manager'), async (req, res) => {
  try {
